...
with tf.device('/device:GPU:1'):
	...
	h_pool1 = max_pool_2x2(h_conv1,"h_pool1") # output size 256x256x32
...
with tf.device('/device:GPU:1'):
	...
	with tf.control_dependencies(update_ops):
		...
		train_step_a = tf.train.AdamOptimizer(lr,name='Adam').minimize(loss * 1.5, var_list =t_v_a)##, var_list =t_v
		train_step_b = tf.train.AdamOptimizer(lr,name='Adam').minimize(loss * 1.3, var_list =t_v_b)##, var_list =t_v
		train_step_c = tf.train.AdamOptimizer(lr,name='Adam').minimize(loss * 1.1, var_list =t_v_c)##, var_list =t_v
...
def train_step_epoch(ii):
	...
	for i in range((restore_step%(STEP_PER_EPOCH_s//batch_size_old))+ii2,(STEP_PER_EPOCH_s//batch_size)+ii2):
		...
		# print(sess.run(prediction, feed_dict = {xs: batch_xs,ys : batch_ys})[0:batch_size//10])
		sess.run(train_step_a, feed_dict = {xs: batch_xs,ys : batch_ys})
		sess.run(train_step_b, feed_dict = {xs: batch_xs,ys : batch_ys})
		# sess.run(train_step_c, feed_dict = {xs: batch_xs,ys : batch_ys})
		# sess.run(train_step_d, feed_dict = {xs: batch_xs,ys : batch_ys})
		# sess.run(train_step_e, feed_dict = {xs: batch_xs,ys : batch_ys})
...
#mppyplot.show()
# nohup python3 -u seamcarving_detect14_float32.py > nohup.log 2>&1 &
#  nohup tensorboard --logdir=:/media/benson/SSD/workspace/logs/ --port=6006> tesorboard.log 2>&1 &
