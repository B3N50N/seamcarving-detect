...
def weight_variable_xavier(shape,names):
    ...
    return tf.Variable(tf.get_variable(name =names, shape=shape,initializer=tf.contrib.layers.xavier_initializer(dtype=tf.float32)))
...
def bias_variable_xavier(shape,names):
    return tf.get_variable(name =names, shape=shape,initializer=tf.contrib.layers.xavier_initializer(dtype=tf.float32))
...
with tf.device('/cpu:0'):
	...
	ys = tf.placeholder(tf.float32, [batch_size,subimage_size//label_scale, subimage_size//label_scale])
...
with tf.device('/device:GPU:1'):#'/device:GPU:0'
	...
	x_image = tf.reshape(xs, [-1, 512, 512, 3])
...
W_conv1_rns = tf.placeholder(tf.float32, [5,5, 3,32])
b_conv1_rns = tf.placeholder(tf.float32, [32])
W_conv2_rns = tf.placeholder(tf.float32, [5,5, 32, 64])
b_conv2_rns = tf.placeholder(tf.float32, [64])
W_conv3_rns = tf.placeholder(tf.float32, [5,5, 64, 128])
b_conv3_rns = tf.placeholder(tf.float32, [128])
W_conv4_rns = tf.placeholder(tf.float32, [5,5, 128, 256])
b_conv4_rns = tf.placeholder(tf.float32, [256])
W_conv5_rns = tf.placeholder(tf.float32, [5,5, 256, 512])
b_conv5_rns = tf.placeholder(tf.float32, [512])
W_conv6_rns = tf.placeholder(tf.float32, [5,5, 512, 1024])
b_conv6_rns = tf.placeholder(tf.float32, [1024])
W_conv7_rns = tf.placeholder(tf.float32, [5,5, 1024, 2048])
b_conv7_rns = tf.placeholder(tf.float32, [2048])
W_conv8_rns = tf.placeholder(tf.float32, [4,4, 2048, 4096])
b_conv8_rns = tf.placeholder(tf.float32, [4096])
W_fc1_rns = tf.placeholder(tf.float32, [2*2*4096, fc_2_node])
...
W_fc6_rns = tf.placeholder(tf.float32, [((subimage_size//label_scale)//2), (subimage_size//label_scale)*(subimage_size//label_scale)])
b_fc6_rns = tf.placeholder(tf.float32, [(subimage_size//label_scale)*(subimage_size//label_scale)])
...
def train_step_epoch(ii):
	...
	for i in range((restore_step%(STEP_PER_EPOCH_s//batch_size_old))+ii2,(STEP_PER_EPOCH_s//batch_size)+ii2):
		...
		## variable ##
		if i % 500 == 0: # 100
			save_path = saver.save(sess, "net12/save_net.ckpt")
...
#mppyplot.show()
# nohup python3 -u seamcarving_detect12.py > nohup.log 2>&1 &
