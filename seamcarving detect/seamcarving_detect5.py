...
return_dict_1 = manager_1.dict()
manager_2 = multiprocessing.Manager()
return_dict_2 = manager_2.dict()
manager_3 = multiprocessing.Manager()
return_dict_3 = manager_3.dict()
def init_reader():
	#read_train variable
	global qfm_t0
	global qfc_t0
...
def read_y(filename,mode,return_dict):
	global subimage_size
	global result_y1
	global result_y2
	global result_y3
...
def read_train(x,y,keep_prob,kkk):
	...
	th1 = Process(target=read_y, args=(filename2,1,return_dict_1))
	th1.start()
	x = read_x(filename,1)
	th1.join()
	
	y_temp = return_dict_1.values()
	y = y_temp[0]
	# np.testing.assert_array_equal(x,read_x(filename,1,return_dict),err_msg='assert fail')
...
def read_test(x,y,keep_prob,kkk):
	...
	# manager = multiprocessing.Manager()
	# return_dict = manager.dict()
	th1 = Process(target=read_y, args=(filename2,2,return_dict_2))
	th1.start()
	x = read_x(filename,2)
	th1.join()
	
	y_temp = return_dict_2.values()
	y = y_temp[0]
...
def read_test_2(x,y,keep_prob,kkk):
	...
	# manager = multiprocessing.Manager()
	# return_dict = manager.dict()
	th1 = Process(target=read_y, args=(filename2,3,return_dict_3))
	th1.start()
	x = read_x(filename,3)
	th1.join()
	
	y_temp = return_dict_3.values()
	y = y_temp[0]
...
# dataset1 = dataset1.batch(batch_size//reapeat_time)
# dataset1 = dataset1.repeat(reapeat_time)
...
for i in range(STEP_PER_EPOCH_s//batch_size * EPOCHS_PRESET):
	...
	## variable ##
	if i % 100 == 0: # 100
		save_path = saver.save(sess, "net5/save_net.ckpt")
...
#mppyplot.show()
# nohup python3 -u seamcarving_detect5.py > nohup.log 2>&1 &
