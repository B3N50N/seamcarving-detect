...
# for i in range(STEP_PER_EPOCH_s):
	# p0= sess.run(next_element1)
	# batch_xs,batch_ys =p0
	# sess.run(x_image, feed_dict = {xs: batch_xs,ys : batch_ys})
	# print(filename)
...
manager_main_1 = multiprocessing.Manager()
return_dict_main_1 = manager_main_1.dict()

p0 =read_train(return_dict_main_1,1)
substart = time.time()
for i in range(STEP_PER_EPOCH_s//batch_size * EPOCHS_PRESET):
	# substart = time.time()
	batch_xs,batch_ys =p0
	th1 = Process(target=read_train, args=(return_dict_main_1,1))
	th1.start()
	...
	## variable ##
	if i % 100 == 0: # 100
		save_path = saver.save(sess, "net7/save_net.ckpt")
	...
	substart = time.time()
	th1.join()
	return_temp = return_dict_main_1.values()
	p0 = return_temp[0]
...
#mppyplot.show()
# nohup python3 -u seamcarving_detect7.py > nohup.log 2>&1 &
