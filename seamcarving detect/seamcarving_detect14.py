...
STEP_PER_EPOCH_s = (test_sample *src*9*fpc) #(sample*((src*noise)+normal)*(preqf+posqf+pure))*(smooth+sharp)
...
def read_train(return_dict_t0,i):
	...
	for i in range(batch_size) :
		#set actual_src
		src_seed = random.randint(0,src-1)
		if src_seed == 0:
			actual_src = 0
		elif src_seed == 1:
			actual_src = 1
	...
	for i in range(batch_size) :
		...
		if qfm_t2 < 2:
			if qfc_t2 < qfc-1:
				if src_t2 <src-1:
					if image_t2 < s_end_t2 -1:
						if fp_t2 < fpc-1:
							filename,filename2 = filename_maker(qfm_t2,str(quality_factor[qfc_t2]),actual_src,image_t2,fp_t2)
							fp_t2 =fp_t2 +1
						elif fp_t2 == fpc-1:
							filename,filename2 = filename_maker(qfm_t2,str(quality_factor[qfc_t2]),actual_src,image_t2,fp_t2)
							fp_t2 = 0
			...
			if src_t2 <src-1:
				if image_t2 < s_end_t2 -1:
					if fp_t2 < fpc-1:
						filename,filename2 = filename_maker_tif(qfm_t2,"pure",actual_src,image_t2,fp_t2)
						fp_t2 =fp_t2 +1
					elif fp_t2 == fpc-1:
						filename,filename2 = filename_maker_tif(qfm_t2,"pure",actual_src,image_t2,fp_t2)
						fp_t2 = 0
...
with tf.device('/cpu:0'):
	...
	ys = tf.placeholder(tf.float16, [batch_size,2])
...
with tf.device('/device:GPU:1'):#'/device:GPU:0'
	...
	## x_image = tf.cast(tf.reshape(xs, [-1, 512, 512, 3]),tf.float16)
...
## fc1 layer ##
with tf.device('/device:GPU:0'):
	W_fc1 = weight_variable([2*2*128*conv_base, fc_2_node],"W_fc1")#65536,3000
	b_fc1 = bias_variable([fc_2_node],"b_fc1")
	h_pool_flat = tf.reshape(h_pool8, [-1, 2*2*128*conv_base])
	h_fc1 = tf.nn.sigmoid(tf.matmul(h_pool_flat, W_fc1) + b_fc1,name="h_fc1")#tf.nn.relu
...
with tf.device('/device:GPU:0'):
	...
	h_fc4 = tf.nn.sigmoid(tf.matmul(h_fc1_drop_1, W_fc4) + b_fc4, name="h_fc4")#tf.nn.relu h_fc3_1
...
with tf.device('/device:GPU:0'):
	...
	prediction = tf.nn.sigmoid(tf.matmul(h_fc4_1, W_fc6) + b_fc6, name="prediction") #tf.nn.sigmoid
...
# W_conv7_rng = tf.where(tf.is_inf(W_conv7_rng2), tf.constant(max_clip,dtype=tf.float16, shape=W_conv7_rng2.shape), W_conv7_rng2)
# b_conv7_rng = tf.where(tf.is_inf(b_conv7_rng2), tf.constant(max_clip,dtype=tf.float16, shape=b_conv7_rng2.shape), b_conv7_rng2)
# W_conv8_rng = tf.where(tf.is_inf(W_conv8_rng2), tf.constant(max_clip,dtype=tf.float16, shape=W_conv8_rng2.shape), W_conv8_rng2)
# b_conv8_rng = tf.where(tf.is_inf(b_conv8_rng2), tf.constant(max_clip,dtype=tf.float16, shape=b_conv8_rng2.shape), b_conv8_rng2)
...
W_conv1_rns = tf.placeholder(tf.float16, [10,10, 3,1*conv_base])
b_conv1_rns = tf.placeholder(tf.float16, [1*conv_base])
W_conv2_rns = tf.placeholder(tf.float16, [7,7, 1*conv_base, 2*conv_base])
b_conv2_rns = tf.placeholder(tf.float16, [2*conv_base])
W_conv3_rns = tf.placeholder(tf.float16, [5,5, 2*conv_base, 4*conv_base])
b_conv3_rns = tf.placeholder(tf.float16, [4*conv_base])
W_conv4_rns = tf.placeholder(tf.float16, [5,5, 4*conv_base, 8*conv_base])
b_conv4_rns = tf.placeholder(tf.float16, [8*conv_base])
W_conv5_rns = tf.placeholder(tf.float16, [5,5, 8*conv_base, 16*conv_base])
b_conv5_rns = tf.placeholder(tf.float16, [16*conv_base])
W_conv6_rns = tf.placeholder(tf.float16, [5,5, 16*conv_base, 32*conv_base])
b_conv6_rns = tf.placeholder(tf.float16, [32*conv_base])
W_conv7_rns = tf.placeholder(tf.float16, [5,5, 32*conv_base, 64*conv_base])
b_conv7_rns = tf.placeholder(tf.float16, [64*conv_base])
W_conv8_rns = tf.placeholder(tf.float16, [4,4, 64*conv_base, 128*conv_base])
b_conv8_rns = tf.placeholder(tf.float16, [128*conv_base])
W_fc1_rns = tf.placeholder(tf.float16, [2*2*128*conv_base, fc_2_node])
b_fc1_rns = tf.placeholder(tf.float16, [fc_2_node])
W_fc4_rns = tf.placeholder(tf.float16, [fc_2_node,((subimage_size//label_scale)//2)])
b_fc4_rns = tf.placeholder(tf.float16, [((subimage_size//label_scale)//2)])
W_fc6_rns = tf.placeholder(tf.float16, [((subimage_size//label_scale)//2), 2])
b_fc6_rns = tf.placeholder(tf.float16, [2])
...
def train_step_epoch(ii):
	...
	for i in range((restore_step%(STEP_PER_EPOCH_s//batch_size_old))+ii2,(STEP_PER_EPOCH_s//batch_size)+ii2):
		...
		## variable ##
		if i % 500 == 0: # 100
			save_path = saver.save(sess, "net14/save_net.ckpt")
...
#mppyplot.show()
# nohup python3 -u seamcarving_detect14.py > nohup.log 2>&1 &
